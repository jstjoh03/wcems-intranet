-- FinalRelease is a cleared status, not a working phase — the
-- transition CASE's catch-all was treating it as P1C_P1 (same bug
-- Perry hit client-side after a P2 promotion). Guard it to NULL so
-- a clean pass writes no gate for a fully credentialed record.
