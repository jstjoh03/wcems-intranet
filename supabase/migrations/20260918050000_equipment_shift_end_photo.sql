-- ═════════════════════════════════════════════════════════════════════
-- EQUIPMENT CHECK-OUT · the end-of-shift check carries the photo
-- (Justin, 2026-09-18, after the second test run)
--
-- On an extended assignment the end-of-shift check now requires a photo
-- of where the gear is being left, and it replaces the separate "final
-- close-out" step — having both was confusing. After the last crew's
-- end-of-shift check a supervisor picks up and returns. Single-night
-- check-outs keep their confirm → close out (photo) flow.
-- Same function as 20260918040000; only the photo rule changes.
-- ═════════════════════════════════════════════════════════════════════
...
begin
  ...
  if p_kind in ('event_closed', 'shift_end') and v_photo is null then
