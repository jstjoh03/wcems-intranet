-- Critical (medication-dose) items no longer hold up a passing exam
-- (Justin, 2026-09-10): a score at or above the passing standard
-- checks the protocol gate and earns the certificate; missed critical
-- items are still recorded on the assignment and surfaced for review,
-- but there is no forced targeted retest.
...
BEGIN
  ...
  IF v_passed THEN
    ...
    IF FOUND THEN
      v_transition := CASE
        WHEN v_rec.working_phase = 'FinalRelease' THEN NULL
      ...
      IF v_transition IS NOT NULL THEN
        ...
        VALUES
          (v_rec.id, v_transition, v_gate, 'complete', v_score || '%', CURRENT_DATE,
           'Auto-checked - portal exam pass (' || v_def.title || ')')
